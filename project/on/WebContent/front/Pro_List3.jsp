	 ...
	 <section class="list a_container">
	     ...
	     <c:if test= "${articleList != null && articleList.size() > 0}">
	     	<c:forEach var= "articleList" items= "${articleList}" varStatus="status">
	     		<c:if test= "${articleList.menu_code == 3}"> 
	     		<!--메뉴코드 3(장신구) 상품만 출력  -->				
			         <article>
	     ...
	     <c:if test="${not empty articleList}">  
		   	<c:choose>
		    	...
		    	<c:otherwise>
		    		<a href="ProList3.fr?menu_code=3&page=${pageInfo.page - 1}">[이전]</a>&nbsp;
		    	</c:otherwise>
		    </c:choose>
		   	<c:forEach var="i" begin="1" end="${pageInfo.maxPage}" >
		   		<c:choose>
		   			...
		   			<c:otherwise>
		   				<a href="ProList3.fr?menu_code=3&page=${i}">[${i}]</a>
		   			</c:otherwise>
		   		</c:choose>
		   	</c:forEach>
		    <c:choose>
		    	...
		    	<c:otherwise>
		    		<a href="ProList3.fr?menu_code=3&page=${pageInfo.page + 1}">[다음]</a>
		    	</c:otherwise>
		    </c:choose>
		    </c:if>
